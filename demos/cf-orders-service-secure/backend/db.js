...
function parseManualCredentials() {
  const { HANA_HOST, HANA_PORT, HANA_USER, HANA_PASSWORD,
          HANA_UAA_URL, HANA_UAA_CLIENTID, HANA_UAA_CLIENTSECRET } = process.env
  if (!HANA_HOST || !HANA_PORT) return null
  // Password credentials
  if (HANA_USER && HANA_PASSWORD)
    return { host: HANA_HOST, port: HANA_PORT, user: HANA_USER, password: HANA_PASSWORD }
  // OAuth credentials (HANA Cloud free tier service key)
  if (HANA_UAA_URL && HANA_UAA_CLIENTID && HANA_UAA_CLIENTSECRET)
    return { host: HANA_HOST, port: HANA_PORT, uaa: { url: HANA_UAA_URL, clientid: HANA_UAA_CLIENTID, clientsecret: HANA_UAA_CLIENTSECRET } }
  return null
}
